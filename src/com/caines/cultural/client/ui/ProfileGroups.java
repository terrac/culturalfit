...
public class ProfileGroups extends Composite  {
	...
	public ProfileGroups() {
		...
		SimpleFront.basicService.getUserProfile(new AsyncCallback<UserProfile>() {
		...
		salary.addChangeHandler(new ChangeHandler() {
			...
			public void onChange(ChangeEvent event) {
				userProfile.salary = Integer.parseInt(salary.getValue(salary.getSelectedIndex()));
				SimpleFront.basicService.sendProfile(userProfile,new AsyncCallback<Void>() {
		...
		zipCode.addChangeHandler(new ChangeHandler() {
			...
			@Override
			public void onChange(ChangeEvent event) {
				userProfile.zipCode = Integer.parseInt(zipCode.getValue());
			}
		});
		SimpleFront.basicService.getUserGroupList(new AsyncCallback<List<UserGroup>>() {
			...
			public void onSuccess(List<UserGroup> result) {
				for(UserGroup ug : result){
				    ...
				    liElement.setInnerHTML(ug.name+"<span class='badge'>"+ug.getPercent()+"%</span>");
				    liElement.addClassName("list-group-item list-width");
