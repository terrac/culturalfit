...
public class BasicServiceImpl extends RemoteServiceServlet implements
	...
	public void setupQuestions(Key<Group> gKey){
		...
		List<UserQuestion> listAnswered=SDao.getUserQuestionDao().getQByProperty("user", li.gUser.getKey()).filter("group",g.getKey()).filter("visited", true).list();
	...
	public Question getNextQuestion() {
		LoginInfo li = LoginService.login(null,null);
		UserQuestion uq=SDao.getUserQuestionDao().getQByProperty("user", li.gUser.getKey()).filter("visited", false).get();
		...
		if(uq == null){
			...
			uq=SDao.getUserQuestionDao().getQByProperty("user", li.gUser.getKey()).filter("visited", false).get();
			if(uq == null){
				return null;
			}
		}
	...
	public void answerQuestion(String answer){
		LoginInfo li = LoginService.login(null,null);
		UserQuestion uq = SDao.getUserQuestionDao().get(li.gUser.currentQuestion);
	...
	public String addQuestion(String question, String answer1, String answer2,
		...
		g.questions.add(SDao.getQuestionDao().put(new Question(question,answer1,answer2,TagUtil.getTagKeys(tagString))));
